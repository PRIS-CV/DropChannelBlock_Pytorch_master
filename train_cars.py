...
trainset    = torchvision.datasets.ImageFolder(root='data/cars/train', transform=transform_train)
trainloader = torch.utils.data.DataLoader(trainset, batch_size=16, shuffle=True, num_workers=4)

testset = torchvision.datasets.ImageFolder(root='data/cars/test', transform=transform_test)
...
if args.model == "resnet50":
    net = resnet50(num_classes=196, cdb_flag=args.cdb)
    pretrained_path = os.path.join(os.path.expanduser('~'), ".torch/models/resnet50-19c8e357.pth")
elif args.model == "vgg19":
    net = vgg19(num_classes=196, cdb_flag=args.cdb)
